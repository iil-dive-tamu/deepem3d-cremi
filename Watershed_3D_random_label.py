...
def watershed_1(prob_b):
    ...
    nlabels = np.arange(1,1+np.unique(labels).max())
    for i in nlabels:
        labels[labels==i] = np.random.randint(1,400)
    return labels
...
def watershed_6(prob):
    ...
    labeled_img,num_labels = skimage.morphology.label(prob, neighbors=4,return_num=True)
    print("num labels watershed= "+str(num_labels))
    
    nlabels = np.arange(1,1+num_labels)
    for i in nlabels:
        labeled_img[labeled_img==i]=np.random.randint(1,400)
    #print(str(np.unique(labeled_img).size))
    #labeled_img=labeled_img.astype('<f4')
    return labeled_img
...
def ImageSliceViews(prob,out_label):
    ...
    class IndexTracker(object):
        def __init__(self, ax, X,Y):
            ...
            #self.imx = ax[0].imshow(self.X[:, :, self.ind],vmin=0, vmax=255,cmap='jet')
            #self.imy = ax[1].imshow(self.Y[:, :, self.ind])
            self.ax.set_title('Segmentation')
            self.ay.set_title('Ground truth')
            
            #self.ax.ticklabel_format(useOffset=False, style='plain')
            #self.ay.ticklabel_format(useOffset=False, style='plain')
